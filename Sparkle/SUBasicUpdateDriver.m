...
{    
    ...
    //FABRICE, if sandbox then go through XPC
    //simple checking. if env dico has APP_SANDBOX_CONTAINER_ID then it is sandboxed.
    if([[[NSProcessInfo processInfo] environment] objectForKey:@"APP_SANDBOX_CONTAINER_ID"])
    {
        if (relaunchPathToCopy != nil) {
            
            SUSandboxUpdate* sandboxupdate = [SUSandboxUpdate new];
            [sandboxupdate setDelegate:self];
            [sandboxupdate setSparkleBundle:self.updater.sparkleBundle];
            [sandboxupdate setRelaunchPathToCopy:[sparkleBundle pathForResource:relaunchToolName ofType:@"app"]];
            [sandboxupdate setHostname:[self.host name]];
            [sandboxupdate setHostbundlePath:[self.host bundlePath]];
            [sandboxupdate setTempDir:self.tempDir];
            [sandboxupdate setProcessIdentifierString:[NSString stringWithFormat:@"%d", [[NSProcessInfo processInfo] processIdentifier]]];
            [sandboxupdate setTargetPath:[self.host.appCachePath stringByAppendingPathComponent:[relaunchPathToCopy lastPathComponent]]];
            [sandboxupdate installWithToolAndRelaunch:relaunch displayingUserInterface:showUI];
            return;
        }
        return;
    }
